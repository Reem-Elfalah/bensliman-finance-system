...
export function CompanyReport({
  ...
  // Filtered transactions for the company (not per customer)
  const filteredTransactions = useMemo(() => {
    ...
    return allTransactions.filter((t) => {
      ...
      // Filter by currency (last part of CustomerName)
      if (selectedCurrency) {
        const customerCurrency = t.CustomerName
          ? t.CustomerName.split(" - ").slice(-1)[0].trim()
          : "";

        if (customerCurrency !== selectedCurrency.trim()) return false;
      }


      // Filter by account (based on city)
      if (selectedAccount) {
        if ((t.country_city || "").trim() !== selectedAccount.trim()) return false;
      }


      // Filter by customer
      if (selectedCustomer) {
        const customerFirstPart = (t.CustomerName || "")
          .split(" - ")[0]
          .trim();

        if (customerFirstPart !== selectedCustomer.trim()) return false;
      }
  ...
  // Calculate totals for each currency (Deposits - Withdrawals)
  ...
  useEffect(() => {
    const fetchCustomers = async () => {
      const { data, error } = await supabase
        ...
        .select("id, customer, account, currency")
  ...
  const handleCustomerClick = (customerName: string) => {
    if (onCustomerClick && customerName && customerName !== "") {
        ...
        >
          <div className="w-full overflow-x-auto">
            ...
            <div className="block print:hidden">
              ...
              {/* Net Totals per Currency (Deposits - Withdrawals) */}
              ...
              <div className="mt-6 p-3 sm:p-4 bg-white border border-gray-200 rounded-lg sm:rounded-xl shadow-sm">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                  {currencies.map((c) => {
                    ...
                    const netTotal = currencyNetTotals.net[currency] || 0;
              ...
              <div className="mb-8">
                ...
                {(!selectedTransactionType ||
                  ...
                  (() => {
                    const entryTransactions = filteredTransactions.filter(
                      (t) =>
                        t.type === "entry" &&
                              ...
                              {entryTransactions.map((t) => (
                                ...
                                >
                                  ...
                                  <td className="border p-1 sm:p-2">
                                    {t.CustomerName ? (
                                      <button
                                        className="text-blue-700 hover:text-blue-900 cursor-pointer text-xs sm:text-sm"
                                        style={{ textDecoration: "none" }}
                                        onClick={() =>
                                          handleCustomerClick(t.CustomerName!)
                                        }
                                      >
                ...
                {(!selectedTransactionType ||
                  ...
                  (() => {
                    const exitTransactions = filteredTransactions.filter(
                      (t) =>
                        t.type === "exit" &&
                ...
                {(!selectedTransactionType ||
                  selectedTransactionType === "buy") &&
                  (() => {
                    const buyFromTransactions = filteredTransactions.filter(
                      (t) => t.type === "buy"
                    );
                                      ...
                                      <div className="space-y-2">
                                        <div className="flex justify-between items-center">
                                          ...
                                          <span className="text-green-600 font-bold">
                                            {data.total.toLocaleString()} {currencyFromTable.name}
                                          </span>
                                        </div>
                ...
                {(!selectedTransactionType ||
                  selectedTransactionType === "sell_to") &&
                  (() => {
                    const sellToTransactions = filteredTransactions.filter(
                      (t) => t.type === "sell_to"
                    );
                                  ...
                                  <td className="border p-1 sm:p-2">
                                    {t.CustomerName ? (
                                      <button
                                        className="text-blue-700 hover:text-blue-900 cursor-pointer text-xs sm:text-sm"
                                        style={{ textDecoration: "none" }}
                                        onClick={() => handleCustomerClick(t.CustomerName!)}
                                      >
                                        {t.CustomerName.split(" - ")[0]}
                                      </button>
                                    ) : (
